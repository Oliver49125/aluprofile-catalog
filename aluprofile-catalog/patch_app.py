...
# 7. Add Lieferant & Dateien section
supplier_section = """
...
# Replace the drawing/photo buttons from Linked Categories
content = re.sub(r'\{safeUrl\(detail\.drawingUrl\).*?\{t\.openPhoto\} <ExternalLink className="h-4 w-4" /></Button>\s*</a>\s*\)}', '', content, flags=re.DOTALL)

# Inject Supplier & Files before Linked Categories
